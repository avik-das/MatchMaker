...
class User(db.Model):
    ...
    def refresh_data(self):
        ...
        for friend in me[u'friends'][u'data']:
            fuser = User(key_name=friend[u'id'],
                    user_id=friend[u'id'], friends=[],
                    access_token=self.access_token,
                    name=friend[u'name'],
                    email=friend.get(u'email'),
                    picture=friend.get(u'picture'))
            fuser.put()
...
class UserMatchHandler(BaseHandler):
    """Show a specific user's compatability, ensure friendship with the logged
    in user"""
    @user_required
    def get(self, user_id):
        if self.user.friends.count(user_id):
            user = User.get_by_key_name(user_id)
            self.render(u'user-match', user=user, match="unknown")
        elif self.user_id == user_id:
            self.render(u'user-match', user=user, is_you=True)
        else:
